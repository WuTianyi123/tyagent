...
def total_token_estimate(messages: List[Dict[str, Any]], *, system_prompt: str = "") -> int:
    ...
    for msg in messages:
        ...
        if isinstance(content, list):
            text_len = sum(
                len(b.get("text", "").encode("utf-8")) for b in content
...
def _serialize_messages(messages: List[Dict[str, Any]]) -> str:
    ...
    for msg in messages:
        ...
        if isinstance(content, list):
            ...
            content = str(content)
        line = content
        # Prepend reasoning content for reasoning models (o1, R1, etc.)
    ...
    while True:
        try:
            ...
            try:
                ...
                logger.error(
                    "Compaction: failed to parse JSON response (permanent failure)"
                )
                return None
            summary = (
                data.get("choices", [{}])[0]
